...
// ─────────────────────────────────────────────
// SCORE DE SAÚDE COMBINADO
// Avalia CPL + tendência de conversas + atingimento de meta
// ─────────────────────────────────────────────
const computeScore = ({ metrics, delta, targets }) => {
  let score = 0;
  let signals = [];

  // 1. CPL vs target (peso 40)
  ...
  // 2. Tendência de conversas (peso 30)
  if (delta?.conversas != null) {
    if (delta.conversas >= 20)      { score += 30; signals.push({ ok: true,  msg: "Conversas crescendo forte" }); }
    else if (delta.conversas >= 0)  { score += 20; signals.push({ ok: true,  msg: "Conversas estáveis" }); }
    else if (delta.conversas >= -15){ score += 8;  signals.push({ ok: false, msg: "Conversas em queda" }); }
    else                            { score += 0;  signals.push({ ok: false, msg: "Conversas caíram muito" }); }
  } else if (metrics.conversas > 0) {
    score += 18;
  }

  // 3. Atingimento de meta (peso 30)
...
// Gera frases acionáveis baseadas em dados reais
// ─────────────────────────────────────────────
const generateInsights = ({ metrics, delta, targets, campaigns }) => {
  const insights = [];

  // 1. Economia vs período anterior
  if (delta?.gasto != null && delta?.conversas != null && metrics.conversas > 0) {
    if (delta.gasto < -5 && delta.conversas >= 0) {
      const economia = Math.abs(delta.gasto);
      insights.push({
        type: "win",
        icon: "💰",
        msg: `Você economizou ${economia.toFixed(0)}% em verba sem perder conversas`,
      });
    }
  }
  ...
  if (delta?.cpl != null && delta.cpl < -5) {
    insights.push({
      type: "win",
      icon: "📉",
      msg: `CPL caiu ${Math.abs(delta.cpl).toFixed(0)}% — sua campanha está mais eficiente`,
    });
  } else if (delta?.cpl != null && delta.cpl > 15) {
    insights.push({
      type: "alert",
      icon: "📈",
      msg: `CPL subiu ${delta.cpl.toFixed(0)}% — revisar criativos e segmentação`,
    });
  }

  // 3. Conversas crescendo
  if (delta?.conversas != null && delta.conversas >= 20) {
    insights.push({
      type: "win",
      icon: "🚀",
      msg: `Conversas cresceram ${delta.conversas.toFixed(0)}% — considere aumentar o investimento`,
    });
  } else if (delta?.conversas != null && delta.conversas <= -20) {
    insights.push({
      type: "alert",
      icon: "⚠️",
      msg: `Conversas caíram ${Math.abs(delta.conversas).toFixed(0)}% — atenção urgente`,
    });
  }
  ...
  if (targets?.target_conversas > 0) {
    const pct = (metrics.conversas / targets.target_conversas) * 100;
    if (pct >= 100) {
      insights.push({ type: "win", icon: "🎯", msg: `Meta de ${fNum(targets.target_conversas)} conversas batida (${pct.toFixed(0)}%)` });
    } else if (pct >= 80) {
      const faltam = targets.target_conversas - metrics.conversas;
      insights.push({ type: "info", icon: "🎯", msg: `Faltam apenas ${fNum(faltam)} conversas pra bater a meta` });
    }
  }

  // 5. Best campaign
  if (campaigns?.length > 0) {
    const withConv = campaigns.filter((c) => c.conversas > 0 && c.cpl > 0);
    if (withConv.length > 0) {
      const best = [...withConv].sort((a, b) => a.cpl - b.cpl)[0];
      insights.push({
        type: "info",
        icon: "🏆",
        msg: `Melhor campanha: "${best.name}" com CPL de R$ ${fBRL(best.cpl)}`,
      });
    }
  }

  // 6. CTR
  if (metrics.ctr < 1 && metrics.impressoes > 1000) {
    insights.push({
      type: "alert",
      icon: "👁️",
      msg: `CTR de ${fPct(metrics.ctr)} está abaixo de 1% — renovar criativos`,
    });
  }

  return insights.slice(0, 4); // máx 4 insights
};
  ...
  return (
    ...
    >
      <div className="flex items-center gap-2 mb-3">
        ...
        <span className="text-[10px] text-zinc-500">· análise inteligente da campanha</span>
      </div>
...
const CampaignHighlights = ({ campaigns }) => {
  ...
  const HighlightCard = ({ label, campaign, color, icon, kind }) => (
    <motion.div
      initial={{ opacity: 0, y: 12 }}
      animate={{ opacity: 1, y: 0 }}
    ...
    >
      ...
      <div className="text-sm font-bold text-zinc-100 truncate mb-2" title={campaign.name}>
        {campaign.name}
      </div>
      <div className="grid grid-cols-3 gap-2">
        ...
        <div>
          <div className="text-[9px] text-zinc-500 uppercase tracking-wider">Conversas</div>
          <div className="text-sm font-mono tabular-nums text-zinc-200">{fNum(campaign.conversas)}</div>
        </div>
        <div>
          <div className="text-[9px] text-zinc-500 uppercase tracking-wider">Gasto</div>
          <div className="text-sm font-mono tabular-nums text-zinc-200">R$ {fBRL(campaign.gasto)}</div>
        </div>
      </div>
    </motion.div>
  );
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
        <HighlightCard label="Melhor performance" campaign={best} color="#10B981" icon="🏆" kind="best" />
        <HighlightCard label="Precisa de atenção" campaign={worst} color="#F59E0B" icon="⚠️" kind="worst" />
      </div>
    </div>
  );
};
          ...
          metrics && (
            <AnimatePresence mode="wait">
              ...
              >
                ...
                {/* SCORE SAÚDE + META PROGRESS + STATUS */}
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-4">
                  <ScoreSaude metrics={metrics} delta={delta} targets={targets} />
                  ...
                  <div className={`${targets.target_conversas > 0 ? "" : "md:col-span-1"}`}>
                    <CplStatus
                      cpl={metrics.cpl}
                ...
                {/* CAMPAIGN HIGHLIGHTS — Best / Worst */}
                {campaignData?.hasData && campaignData.campaigns?.length > 1 && (
                  <CampaignHighlights campaigns={campaignData.campaigns} />
                )}
