...
let _gmbAccountFetchLock      = null;  // Promise lock — prevents concurrent account fetches
...
export async function getDefaultAccountName() {
  // 1. Memory cache hit — fastest path, no API call
  if (_gmbAccountName) return _gmbAccountName;

  // 2. Already fetching — return the in-flight promise instead of making a second request
  ...
  _gmbAccountFetchLock = (async () => {
    try {
      // Honour quota backoff — if a recent call failed with quota exceeded, don't retry yet
      const now = Date.now();
      if (_gmbAccountQuotaBackoff > now) {
        ...
        throw Object.assign(new Error(`mybusinessaccountmanagement backoff — retry in ${waitSec}s`), { isBackoff: true });
      }
...
export async function getGmbReviews(locationRaw, pageSize = 10) {
  ...
  // Extract just the numeric location ID — mybusinessreviews v1 accepts "locations/{id}"
  // without the account prefix, which avoids calling mybusinessaccountmanagement entirely.
  ...
  if (/^\d+$/.test(trimmed)) {
    ...
    if (!m) throw new Error(`gmb_location_id inválido: "${trimmed}".`);
    locationId = m[1];
  }

  const locationName = `locations/${locationId}`;

  // Use the new Business Profile Reviews API (v1) — mybusiness.googleapis.com/v4 is deprecated
  const response = await axios.get(
...
/**
 * Extracts daily time series for charts: returns array of
 * { date: "YYYY-MM-DD", ligacoes, cliquessite, direcoes, impressoes }
 * sorted ascending by date.
 */
export function extractGmbDailySeries(series) {
  const byDate = new Map();

  const addToDate = (date, key, value) => {
    if (!byDate.has(date)) {
      byDate.set(date, { date, impressoes: 0, ligacoes: 0, cliquessite: 0, direcoes: 0 });
    }
    byDate.get(date)[key] += Number(value || 0);
  };
  ...
  for (const entry of series) {
    ...
    for (const pt of points) {
      ...
      switch (metric) {
        case "CALL_CLICKS":               addToDate(dateStr, "ligacoes", pt.value); break;
        case "WEBSITE_CLICKS":            addToDate(dateStr, "cliquessite", pt.value); break;
        case "BUSINESS_DIRECTION_REQUESTS": addToDate(dateStr, "direcoes", pt.value); break;
        case "BUSINESS_IMPRESSIONS_DESKTOP_MAPS":
        case "BUSINESS_IMPRESSIONS_DESKTOP_SEARCH":
        case "BUSINESS_IMPRESSIONS_MOBILE_MAPS":
        case "BUSINESS_IMPRESSIONS_MOBILE_SEARCH":
          addToDate(dateStr, "impressoes", pt.value);
          break;
        default: break;
      }
    }
