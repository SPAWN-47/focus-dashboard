...
export async function getGmbReviews(locationRaw, pageSize = 10) {
  ...
  // Resolve to full "accounts/.../locations/..." path required by reviews API
  const { locationName } = await resolveLocationId(locationRaw);
...
export async function getGmbInsights(locationRaw, startDate, endDate) {
  const { locationId } = await resolveLocationId(locationRaw);
  const token = await getGmbAccessToken();
