 ...
 */
export async function getGmbReviews(locationName, pageSize = 10) {
  const token = await getGmbAccessToken();
 ...
 * @param {string} locationName   - "locations/{locationId}" (no account prefix)
 * @param {Date}   startDate
 * @param {Date}   endDate
 * Returns raw API response data
 */
export async function getGmbInsights(locationId, startDate, endDate) {
  const token = await getGmbAccessToken();
