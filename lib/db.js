...
export function seedClients() {
  ...
  try {
    ...
    // Only seed when there are zero clients (fresh DB / first deploy)
    ...
    // Regardless of whether we seeded, sync gmb_location_id & google_ads_customer_id
    // from seed file to existing rows (INSERT OR IGNORE skips existing rows,
    // so these fields would never update otherwise).
    const syncIds = db.prepare(`
      UPDATE clients
      SET
        gmb_location_id        = COALESCE(NULLIF(@gmb_location_id, ''), gmb_location_id),
        google_ads_customer_id = COALESCE(NULLIF(@google_ads_customer_id, ''), google_ads_customer_id)
      WHERE id = @id
