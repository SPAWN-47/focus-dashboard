...
/**
 * Seeds clients from config/clients.seed.json on every startup.
 * Uses INSERT OR IGNORE so existing clients (with real tokens/data) are never overwritten.
 * This ensures the dashboard always has base clients after a fresh deploy.
 */
  ...
  try {
    ...
    const insert = db.prepare(`
      INSERT OR IGNORE INTO clients
        (id, name, emoji, color, token, account_id, google_ads_customer_id)
      VALUES
        (@id, @name, @emoji, @color, @token, @account_id, @google_ads_customer_id)
    `);
