...
export async function queryGoogleAds(customerId, query) {
  ...
  // Validate: must be 10 digits exactly
  if (!/^\d{10}$/.test(cid)) {
    throw new Error(`Customer ID inválido: "${customerId}". Deve conter 10 dígitos (ex: 123-456-7890).`);
  }
...
export function computeGoogleMetrics(rows) {
  ...
  for (const row of rows) {
    ...
    // search_impression_share is a fraction 0–1 (e.g. 0.72 = 72%)
    const isRaw = Number(m.searchImpressionShare ?? m.search_impression_share ?? -1);
    if (isRaw >= 0) {
      impressionShareSum += isRaw;
  ...
  // Search Impression Share as percentage (null when not available)
  const impressionShare = impressionShareCount > 0
    ? (impressionShareSum / impressionShareCount) * 100
    : null;

  return { gasto, impressoes, cliques, conversas, cpl, ctr, cpm, cpc, impressionShare };
}
